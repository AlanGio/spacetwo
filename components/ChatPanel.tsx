...
const mockMessages: ChatMessage[] = [
  {
    ...
    message: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Excepteur sint non proident",
  ...
  },
  {
    ...
    message: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cupidatat non proident",
  ...
  },
  {
    ...
    message: "Labore et dolore magna aliqua. Ut enim ad minim veniam, laboris nisi ut aliquip ex ea commodo consequat.",
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-[#333333]">
        <h3 className="text-[#827989] text-sm font-medium"># {projectName}</h3>
        <Button variant="ghost" size="sm" onClick={onClose} className="text-[#827989] hover:text-white">
          <X className="w-4 h-4" />
        </Button>
      </div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="font-medium" style={{ color: message.user.color }}>
                    {message.user.name}
                  </span>
                  <span className="text-xs text-[#827989]">{message.timestamp}</span>
                </div>
                ...
                {message.reply && (
                  <div className="bg-[#333333] rounded-lg p-3 mb-2 border-l-2 border-[#666666]">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      />
                      <span className="text-xs font-medium" style={{ color: message.user.color }}>
                        {message.reply.user}
                      </span>
                    </div>
                    <p className="text-sm text-[#999999]">{message.reply.message}</p>
                  </div>
                )}

                <p className="text-sm text-[#cccccc] leading-relaxed">
                  {message.message}
                  {message.emoji && <span className="ml-2 text-base">{message.emoji}</span>}
                </p>
              </div>
            </motion.div>
          ))}
