...
export default function AddNewPanel({ projectName, isOpen, onClose, availableProjects = [] }: AddNewPanelProps) {
  const [embedUrl, setEmbedUrl] = useState("https://adobe.premiere.com/Oe-tP6z...")
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        <div className="space-y-3">
          ...
          <p className="text-[#827989] text-sm">Works with Figma, Adobe, Notion and more</p>
        </div>
        ...
        <div className="space-y-2 relative">
          <div
            ...
            onClick={() => availableProjects.length > 0 && setIsProjectDropdownOpen(!isProjectDropdownOpen)}
          >
            <span className={availableProjects.length > 0 ? "text-white" : "text-[#827989]"}>
              {availableProjects.length > 0 ? selectedProject : "No projects available"}
            </span>
            {availableProjects.length > 0 && (
              <ChevronDown
                className={`w-4 h-4 text-[#827989] transition-transform ${isProjectDropdownOpen ? "rotate-180" : ""}`}
              />
            )}
          </div>
          ...
          {/* Dropdown Menu */}
          <AnimatePresence>
            {isProjectDropdownOpen && availableProjects.length > 0 && (
              <motion.div
