...
export default function CommunityCard({ card, className = "col-span-1", index, activeFilter }: CommunityCardProps) {
  ...
  return (
    <motion.div
      ...
      custom={index}
    >
      <div className={`group cursor-pointer ${isNavigating ? "pointer-events-none" : ""}`} onClick={handleCardClick}>
        <div
        ...
        >
          ...
          >
            <div className="w-8 h-8 bg-[#333333] rounded flex items-center justify-center text-xs">{card.appIcon}</div>
          </motion.div>
          <motion.div
          ...
          >
            <div className="px-2 py-1 bg-black/50 rounded text-xs text-white">{card.category}</div>
          </motion.div>
        </div>
        <motion.div
